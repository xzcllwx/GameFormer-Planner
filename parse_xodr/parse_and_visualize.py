...
import os
import sys
sys.path.append(os.path.abspath('/root/xzcllwx_ws/GameFormer-Planner/pyeulerspiral/'))
sys.path.append(os.path.abspath('/root/xzcllwx_ws/GameFormer-Planner/parse_xodr'))
...
XODR_FILE = "/root/xzcllwx_ws/GameFormer-Planner/parse_xodr/make.xodr"
...
def calculate_area_of_one_left_lane(left_lane, points, most_left_points):
    ...
    for reference_point, inner_point in zip(points, inner_points):
        ...
        x_center = x_inner + cos(normal_left) * lane_width_offset / 2.0
        y_center = y_inner + sin(normal_left) * lane_width_offset / 2.0
...
def calculate_area_of_one_right_lane(right_lane, points, most_right_points):
    ...
    for reference_point, inner_point in zip(points, inner_points):
        ...
        x_center = x_inner + cos(normal_eight) * lane_width_offset / 2.0
        y_center = y_inner + sin(normal_eight) * lane_width_offset / 2.0
...
def get_lanes_by_road_id(road_network, road_id, step=0.1):
    """
    Get all lanes of one road network.
    :param road_network: Parsed road network.
    :param step: Step of calculation.
    ...
    for road in tqdm(roads, desc="Calculating boundary points."):
        # print(f"road id: {road.id}, road_id: {road_id}")
        # print(type(road.id), type(road_id))
        if road.id != road_id:
            continue
        lanes_of_one_road = get_lane_area_of_one_road(road, step=step)
        total_areas_all_roads = {**total_areas_all_roads, **lanes_of_one_road}
...
def process_one_file(file, step=0.1):
    ...
    total_areas = get_lanes_by_road_id(road_network, 110, step=step)

    plot_planes_of_roads(total_areas, save_folder)
    
def get_lane_points_by_id(file=XODR_FILE, step=0.1):
    """
    Load one .xodr file and calculate the railing positions with other important messages.
    :param file: Input file.
    :param step: Step of calculation.
